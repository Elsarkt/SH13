...
int main(int argc, char *argv[])
{
	...
	// gération nombre random (basé sur heure actuelle)
	srand(time(NULL));
	...
	else if (fsmServer==1)
	{
		switch (buffer[0])
		{
                case 'G': // Cas guilty
					...
					int accuser, suspect;
					sscanf(buffer, "%c %d %d", &com, &accuser, &suspect);
					
					if (suspect == deck[12]) { // Vérif accusation
						//printf("%s gagne. %s accusé correctement. !\n", tcpClients[accuser].name, nomcartes[suspect]);
						sprintf(reply, "%s gagne. %s accusé correctement. !\n", tcpClients[accuser].name, nomcartes[suspect]);
						broadcastMessage(reply);
					} else {
						joueurCourant = (joueurCourant + 1) % 4; // Mauvaise accusation, joueur suivant
						sprintf(reply, "M %d", joueurCourant);
						broadcastMessage(reply);
					}
					break;
						
				case 'O': // Cas Observation
					// RAJOUTER DU CODE ICI				
					int observer; 
					int type;
					sscanf(buffer, "%c %d %d", &com, &observer, &type);
					
					for (i = 0; i < 4; i++) { 
						if (i != observer) {
							if (tableCartes[i][type] == 0) { // Si les joueurs n'ont pas le typed de symbol demandé
								sprintf(reply, "V %d %d %d", i, type, 0);
								broadcastMessage(reply);
							}
						}
					}
				...
				case 'S': // Cas Spécifié 
					...
					int asked; 
					int symbol;
					sscanf(buffer, "%c %d %d %d", &com, &asker, &asked, &symbol);
					
					sprintf(reply, "V %d %d %d", asked, symbol, tableCartes[asked][symbol]); // Nombre de symbole symbol possédé par asked envoyé 
					broadcastMessage(reply);
