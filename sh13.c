...
void *fn_serveur_tcp(void *arg)
{
       ...
       if (bind(sockfd, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0)
        ...
        while (1)
        {
				...
				// Protection de l'accès à gbuffer avec mutex
				pthread_mutex_lock(&mutex); // ? pk libérer gbuffer ?
                bzero(gbuffer,256);
                n = read(newsockfd,gbuffer,255);
                if (n < 0)
                {
                        printf("read error\n");
						pthread_mutex_unlock(&mutex); // Libérer le mutex en cas d'erreur ?
                        exit(1);
                }
                //printf("%s",gbuffer);

				// Signaler au thread principal qu'un message est disponible
                synchro=1;
				pthread_mutex_unlock(&mutex);

                // while (synchro); // ? mis en commentaire mais est-ce pertinent ?
				// Attendre que le thread principal traite le message
				int done = 0;
				while (!done)
				{
	...
	if (SDL_PollEvent(&event))
	{
        ...
        if (synchro==1)
        {
			...
			switch (gbuffer[0])
			{
				...
				case 'L':
					// RAJOUTER DU CODE ICI
					sscanf(gbuffer + 2, "%s %s %s %s", 
					gNames[0], gNames[1], gNames[2], gNames[3]);
					printf("Liste des joueurs mise à jour\n");
					break;
				...
				case 'V':
					// RAJOUTER DU CODE ICI
					int player, object, value;
					sscanf(gbuffer + 2, "%d %d %d", &player, &object, &value);
					
					if (tableCartes[player][object] == -1) { // Ecris seulement si on ne connaît pas la value
						tableCartes[player][object] = value; //Inscrit le nb de symbol considéré possédé par player
						printf("Mise à jour de la table: Joueur %d, Objet %d, Valeur %d\n", 
							player, object, value);
					}
					break;
			}

			// Réinitialiser le signal après traitement
			synchro = 0; // Signaler msg traité au serveur
			pthread_mutex_unlock(&mutex);
        }
